"""No-regen sweep with a power cap applied to the existing torque command.

This keeps the current LVCU/pedal/inverter behavior and inserts only:

    torque_command = min(torque_command, power_limit / motor_omega)

before inverter delivery. It is meant to answer the practical tuning
question: what power limit should be applied to commanded torque?
"""
...
class CommandPowerLimitedPowertrain:
    """Delegate wrapper that caps the positive LVCU torque command."""

    def __init__(self, base: Any, power_kw: float) -> None:
        self._base = base
    ...
    def _cap_command(self, motor_rpm: float, torque_nm: float) -> float:
        torque = float(torque_nm)
        if torque <= 0.0:
            return torque
        rpm = max(0.0, float(motor_rpm))
        if self._power_w <= 0.0:
            return 0.0
        if rpm <= 1e-6:
            return torque
        omega = rpm * 3.141592653589793 / 30.0
        return min(torque, self._power_w / omega)
    ...
    def lvcu_torque_command(
        ...
        **kwargs: Any,
        ...
        state = self._base.lvcu_torque_command(
            pedal_pct,
            motor_rpm,
            bms_current_limit_a,
            **kwargs,
        ...
        if kwargs.get("return_state", False):
            return replace(
                state,
                torque_nm=self._cap_command(motor_rpm, state.torque_nm),
            )
        return self._cap_command(motor_rpm, float(state))
    ...
    def lvcu_torque_ceiling(
        ...
        return self._cap_command(
            motor_rpm,
            self._base.lvcu_torque_ceiling(motor_rpm, bms_current_limit_a),
        )
    ...
    def pedal_to_torque_request(
        ...
        return self._cap_command(
            motor_rpm,
            self._base.pedal_to_torque_request(
                pedal_remapped,
                motor_rpm,
                bms_current_limit_a,
            ),
...
def run_case(torque_nm: float, rpm: int, power_kw: float) -> dict[str, Any]:
    ...
    try:
        with warnings.catch_warnings():
            ...
            engine.powertrain = CommandPowerLimitedPowertrain(engine.powertrain, power_kw)
...
def _write_outputs(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    fields = [
        "torque_nm", "rpm", "power_limit_kw", "power_knee_rpm",
        "combined_score", "endurance_total", "efficiency_score",
        "efficiency_factor", "time_s", "net_kwh", "discharge_kwh",
        "regen_kwh", "brake_energy_kwh", "final_soc_pct",
        "laps_completed", "peak_speed_kmh", "peak_motor_rpm",
        "peak_motor_torque_nm", "peak_pack_current_a", "max_cell_temp_c",
        "speed_limited_pct", "speed_limit_violations", "throttle_segments",
        "brake_segments", "coast_segments", "duration_s",
    ]
    with (out_dir / "command_power_limit_sweep.csv").open(
    ...
    (out_dir / "command_power_limit_sweep.json").write_text(
        json.dumps({"rows": rows, "best": ranked[0]}, indent=2),
        encoding="utf-8",
    )
    best = ranked[0]
    lines = [
        "# Command Power-Limit Sweep",
        "",
        "Model: adaptive friction-only braking, existing command path with "
        "`torque_command = min(torque_command, P/omega)`. Full 22-lap "
        "endurance, 2025 Michigan scoring, no penalties.",
        "",
        "## Recommendation",
        "",
        (
            f"- Best: `{best['torque_nm']:.1f} Nm`, `{best['rpm']} rpm`, "
            f"`{best['power_limit_kw']:.2f} kW` = "
        ...
        "## Top 20",
        "",
        "| Rank | Torque | RPM | Power kW | Knee RPM | Points | Time | Net kWh | Max Temp C | Peak A |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for idx, row in enumerate(ranked[:20], start=1):
        lines.append(
            f"| {idx} | {row['torque_nm']:.1f} | {row['rpm']} | "
            f"{row['power_limit_kw']:.2f} | {row['power_knee_rpm']:.0f} | "
            f"{row['combined_score']:.3f} | {row['time_s']:.1f} | "
            f"{row['net_kwh']:.3f} | {row['max_cell_temp_c']:.1f} | "
            f"{row['peak_pack_current_a']:.1f} |"
        )
    (out_dir / "command_power_limit_sweep.md").write_text(
        "\n".join(lines) + "\n",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    parser.add_argument("--torques", type=float, nargs="*", default=[72.5, 75.0, 77.5, 80.0, 82.5, 85.0])
    parser.add_argument("--rpms", type=int, nargs="*", default=[3300, 3400, 3500, 3600])
    parser.add_argument("--power-kw", type=float, nargs="*", default=[22.0, 24.0, 26.0, 28.0, 30.0, 32.0])
    args = parser.parse_args()

    out_dir = args.out_dir or (
        REPO / "results" / f"command_power_limit_sweep_{datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    print(
        f"best={best['torque_nm']:.1f}Nm {best['rpm']}rpm "
        f"{best['power_limit_kw']:.2f}kW {best['combined_score']:.3f}pts"
    )
    print(f"wrote {out_dir / 'command_power_limit_sweep.md'}")
    return 0
